...
function Join() {
  ...
  const handleCheck = (e) => {
    ...
    if (name === "id" || name === "email" || name === "phone") {
      ...
      fetch("/api/member/check", {
        ...
        .then((res) => {
          if (res.num > 0) {
            next.innerHTML = `이미 존재하는 ${
              name !== "phone" ? name : "핸드폰 번호"
            }입니다`;
          }
        });
    } else {
  ...
  const handleJoin = (e) => {
    ...
    for (let i = 0; i < now.childNodes.length - 1; i++) {
      if (i === 7) {
        handleCheck(now.childNodes[i].childNodes[0]);
        handleCheck(now.childNodes[i].childNodes[1]);
        handleCheck(now.childNodes[i].childNodes[2]);
      } else if ((i < 3 && i % 2 === 0) || (i >= 3 && i % 2 === 1)) {
